...
export default defineComponent({
	...
	setup() {
		...
		const getCaptcha = () => {
			state.codeParam.action = OtherUtil.randomPassword(6).str;
			state.codeParam.appid = OtherUtil.randomPassword(6).str;
			state.codeParam.nowTime = new Date().getTime();
			state.captchaImg = `${baseUrl}${SERVER_NAME.SECURITY_SERVER}servlet/patchcaServlet?action=${state.codeParam.action}&appid=${state.codeParam.appid}&key=${state.codeParam.nowTime}`;
			state.ruleForm.queryParam = state.codeParam.action + state.codeParam.appid + state.codeParam.nowTime;
		}
		// 时间获取
